...
def save_debug_dump(page: Page, debug_dir: Path) -> None:
    ...
    try:
        ...
        pass
    logger.info("debug_dump_saved", dump_dir=str(dump_dir))


@contextmanager
def tenbis_context(
    profile_dir: Path, headless: bool, debug_dir: Path
) -> Iterator[tuple[BrowserContext, Page]]:
    """Yield a (context, page) pair for 10bis using a persistent Chromium profile."""
    with sync_playwright() as p:
        ctx = launch_browser(p, profile_dir, headless)
        page = ctx.new_page()
        try:
            yield ctx, page
        except Exception:
            save_debug_dump(page, debug_dir)
            raise
        finally:
            ctx.close()
...
def launch_browser(p: Playwright, user_data_dir: Path, headless: bool) -> BrowserContext:
